...
- **Design bans (revised Phase 13, 2026-06-03 — identity-preservation OFF, Jamie ruling)**: still banned — gradient text, glassmorphism, em-dashes in copy (hyphens in number-word compounds such as "30-min" are allowed). **No longer hard bans, now available with judgement** (DESIGN.md governs): card shadows (elevation earned), mid-tone greys (prefer faded brand colours over true grey), decorative card grids (avoid the generic four-icon grid by taste), font-weight 500 (door open per D-09). The "if any AI tool could have made it, it failed" test still applies.
